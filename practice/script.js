class TypingTest {
  constructor() {
    ...
    this.textDatabase = {
      easy: [
        "Bright lanterns lit the night market as laughter echoed through the narrow alleys.",
        "He flipped through the old journal, each page filled with memories and faded ink.",
        "A silver train sliced through the morning fog, vanishing into the horizon like a ghost.",
        "The library smelled of old books and rain, a quiet refuge from the bustling world outside.",
      ],
      medium: [
      ...
      expert: [
        ...
        "Despite the extraordinary promise of CRISPR and other gene-editing technologies to eradicate hereditary diseases, the scientific community remains deeply divided on the moral ramifications of germline modification, fearing unintended consequences that could ripple across generations and ecosystems.",
      ],
    };
  ...
  addComplexity(text, difficulty) {
    ...
    return words
      .map((word) => {
        if (Math.random() < 0.1) {
          return (
            complexity[Math.floor(Math.random() * complexity.length)] || word
          );
        }
        return word;
      })
  ...
  savePracticeTime() {
    localStorage.setItem(
      "totalPracticeTime",
      this.totalPracticeTime.toString()
    );
  }
  ...
  updateStatistics() {
    ...
    const avgWpm =
      recentTests.length > 0
        ? Math.round(
            recentTests.reduce((sum, test) => sum + test.wpm, 0) /
              recentTests.length
          )
        : 0;
    ...
    this.elements.practiceTime.textContent = `${Math.round(
      this.totalPracticeTime / 60
    )} min`;
  }
}
